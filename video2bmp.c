...
{ 
	...
	/* 进行转换 frame->data和pFrameRGB->data分别指输入和输出的buf frame->linesize和pFrameRGB->linesize可以看成是输入和输出的每列的byte数 第4个参数是指第一列要处理的位置 第5个参数是source slice 的高度 */ 
...
{ 
	...
	// 打印输入文件的详细信息 
	av_dump_format(fmt_ctx, 0, filename, 0); av_init_packet(&avpkt); 
	...
	srcW/srcH/srcFormat分别为原始图像数据的宽高和数据类型，数据类型
	比如AV_PIX_FMT_YUV420、PAV_PIX_FMT_RGB24 dstW/dstH/dstFormat分别为输出图像数据的宽
	高和数据类型 flags是scale算法种类；SWS_BICUBIC、SWS_BICUBLIN、SWS_POINT、SWS_SINC等 
	最后3个参数不用管，都设为NULL 
	*/ 
